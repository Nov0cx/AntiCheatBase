...
public enum BlockData {
    ...
    public CollisionBox getBox(Block block, ProtocolVersion version) {
        ...
        CollisionBox fetched = dynamic.fetch(version, block);
        return fetched;
    }

    public SimpleCollisionBox getBoxOrFirstOfComplex(Block block, ProtocolVersion version) {
        CollisionBox collisionBox = getBox(block, version);
        if (collisionBox instanceof SimpleCollisionBox) return (SimpleCollisionBox) collisionBox;
        if (collisionBox instanceof ComplexCollisionBox)
            return (SimpleCollisionBox) ((ComplexCollisionBox) collisionBox).getBoxes().get(0);
        return null;
    }
