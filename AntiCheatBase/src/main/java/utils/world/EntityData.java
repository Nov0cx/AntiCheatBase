    ...
    public static CollisionBox bounds(Entity entity) {
        return entityBounds.computeIfAbsent(entity.getType(), type -> {
            if(ProtocolVersion.getGameVersion().isBelow(ProtocolVersion.V1_14)) {
                Object ventity = Reflections.getCBClass("entity.CraftEntity").getMethod("getHandle").invoke(entity);
                ...
                WrappedClass ce = Reflections.getCBClass("entity.CraftEntity");
                return new SimpleCollisionBox(new Vector(), ce.getMethod("getWidth").invoke(entity), ce.getMethod("getHeight").invoke(entity));
            }
        }).copy();
    }
