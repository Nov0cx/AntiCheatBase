...
public class BoundingBox {
    ...
    public List<BoundingBox> getAllBoxes(World world, ProtocolVersion version) {
        Location min = new Location(world, MathUtils.floor(minX), MathUtils.floor(minY), MathUtils.floor(minZ));
        Location max = new Location(world, MathUtils.floor(maxX), MathUtils.floor(maxY), MathUtils.floor(maxZ));
        ...
        for (float x = (float) min.getX(); x < max.getX(); x++) {
            for (float y = (float) min.getY(); y < max.getY(); y++) {
                for (float z = (float) min.getZ(); z < max.getZ(); z++) {
                    ...
                    if (block != null && block.getType().getId() != 0) {
                        CollisionBox box = BlockData.getData(block.getType()).getBox(block, version);

                        if (box instanceof SimpleCollisionBox) {
                            all.add(((SimpleCollisionBox) box).toBoundingBox());
                        } else if (box instanceof ComplexCollisionBox) {
                            all.add(((SimpleCollisionBox) ((ComplexCollisionBox) box).getBoxes().get(0)).toBoundingBox());
                        }
    ...
    public BoundingBox addMax(float x, float y, float z) {
        this.maxX += x;
        this.maxY += y;
        this.maxZ += z;
        return this;
