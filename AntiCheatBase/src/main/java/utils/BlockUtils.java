...
import cc.funkemunky.api.utils.types.WrappedField;
...
public class BlockUtils {
    ...
    public static boolean isClimbable(Block b) {
        XMaterial mat = XMaterial.fromString(b.getType().toString());
        return mat == XMaterial.LADDER || mat == XMaterial.VINE;
    }
    ...
    static {
        ...
        if (ProtocolVersion.getGameVersion().isOrAbove(ProtocolVersion.V1_13)) {
            fieldBlocksMovement = (ProtocolVersion.getGameVersion().isOrAbove(ProtocolVersion.v1_16)
                    ? Reflections.getNMSClass("BlockBase$Info") : Reflections.getNMSClass("Block$Info")).getFieldByType(boolean.class, 0);
        }
    }
}
